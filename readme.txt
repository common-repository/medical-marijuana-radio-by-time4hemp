=== Medical Marijuana Radio===
Contributors: WeedBiz, Spreaker
Tags: spreaker, marijuana, audio, embed, widget, player, podcast, music, sound, cannabis, medical marijuana, hemp
Requires at least: 2.5.0
Tested up to: 4.4
Stable tag: 1.1


A quick and easy way to embed 24/7 medical marijuana and hemp radio into your WordPress blog.


== Description ==

The Medical Marijuana Radio plugin is an easy way to embed a Spreaker audio player that plays medical marijuana and hemp-related programming from a network of medical-marijuana related radio stations in your Wordress sidebar. This plugin is a modified version of the [Spreaker Shortcode plugin](https://wordpress.org/plugins/spreaker-shortcode/), and modifies the original plugin to add styling and station settings specific to this station.

Example Usage:

`[medical-marijuana-radio]`


**More Options**

Medical Marijuana Radio supports these optional parameters:

* `type`: `standard` or `mini` (defaults to `mini`)
* `autoplay`: if `true` the player will automatically start playing (defaults to `false`)
* `debug`: if `true` will print some debug information, which is useful if the player renders differently than expected (defaults to `false`)

**User Customization**

The Medical Marijuana Radio administrative menu link gives an option for the administrator to include custom css that will affect the front-end display for the plugin.


**Help**

If you need further help, please contact us at [WeedBiz.us](http://www.weedbiz.us).

== Installation ==

* Upload plugin to the /wp-content/plugins/ directory
* Activate the plugin through the 'Plugins' menu in WordPress
* Add the shortcode [medical-marijuana-radio] to a text widget in your Wordpress sidebar.

== Screenshots ==

1. Admin view
2. Front end view

== Changelog ==

= 1.2 =
* Removed Time4Hemp, updated station identifier

= 1.1 =
* Replaced 'episode_id' with 'show_id' to prevent "Station Offline" errors

= 1.0 =
* First Release
